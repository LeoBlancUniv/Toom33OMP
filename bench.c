		...
		mp_limb_t *soak, mp_limb_t *c_limbs, mp_limb_t *soak2, int nb_limbs)
{
	...
	if (mpn_cmp(x, c_limbs, 2 * nb_limbs) != 0){
		...
		/*gmp_printf("%Nx ", c_limbs, nb_limbs);
		gmp_printf("%Nx \n", x, nb_limbs);*/
...
void do_benchgmp(uint64_t retcycles[3], const char* pstr, const uint8_t W)
{
	...
	retcycles[0] = gmpbench(A, B, modul_p, r, c_limbs, q_limbs, W, gmp_toom3_wrapper);
...
int main(void)
{
	scratch2 = calloc(1024, sizeof(mp_limb_t));
