...
static inline void gmp_test_wrapper(mp_limb_t *a_limbs, mp_limb_t *b_limbs,
		mp_limb_t *soak, mp_limb_t *c_limbs, mp_limb_t *soak2, int nb_limbs){

		UNUSED(soak);
		UNUSED(soak2);

		test(a_limbs, b_limbs, nb_limbs, c_limbs);
...
void do_benchgmp(uint64_t retcycles[3], const char* pstr, const uint8_t W)
{
	...
	retcycles[3] = gmpbench(A, B, modul_p, r, c_limbs, q_limbs, W, gmp_test_wrapper);
